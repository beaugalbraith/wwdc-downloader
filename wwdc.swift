...
/*
	...
	Date: June 17th 2017
	About this script:
 WWDC 2013 is ending and even if there are some great tools out there (https://github.com/insidegui/WWDC) that allow to see and download video sessions,
...
class wwdcVideosController {
    class func getHDorSDdURLs(fromHTML: String, format: VideoQuality) -> (String) {
        let pat = "\\b.*(http[s]?://.*" + format.rawValue + ".*\\.m[po][v4])\\b"
    ...
    class func getSessionsList(fromHTML: String) -> Array<String> {
        let pat = "\"\\/videos\\/play\\/wwdc\(YEAR)/([0-9]*)\\/\""
        let regex = try! NSRegularExpression(pattern: pat, options: [])
        let matches = regex.matches(in: fromHTML, options: [], range: NSRange(location: 0, length: fromHTML.count))
...
func showHelpAndExit() {
    print("wwdc\(YEAR) - a simple swifty video sessions bulk download.\nJust Get'em all!")
    print("usage: wwdc\(YEAR).swift [--hd] [--sd] [--pdf] [--pdf-only] [--sessions] [--sample] [--list-only] [--help]\n")
    exit(0)
}
...
let htmlSessionListString = wwdcVideosController.getStringContent(fromURL: "https://developer.apple.com/videos/wwdc\(YEAR)/")
...
for (_, value) in sessionsListArray.enumerated() {
    let htmlText = wwdcVideosController.getStringContent(fromURL: "https://developer.apple.com/videos/play/wwdc\(YEAR)/" + value + "/")
